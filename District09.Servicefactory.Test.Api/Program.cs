...
builder.Services.AddMessaging(builder.Configuration,
    opts =>
    {
        opts
            ...
            .WithListener<MyData,
                MyMessageHandler>("some.queue.mydata") // only after middleware is executed, handle the message
            .WithPublisher<MyData>("some.queue.mydata");

        opts.WithListener<MySecondData, MySecondHandler>("some.queue.seconddata")
            .WithListenerMiddleware<MySecondMiddleware,
                MySecondData>() // in this case, middleware is executed after the message handler
